...
-----------------------------------------------------------------------------------------------------------------------------
-------------------------------------------- DROP FUNCTION ------------------------------------------------------------------
 DROP FUNCTION getDataValueInStageByProgramInstance( _programinstanceid integer, _programstageuid text, _dataelementcode text) ;
...
CREATE OR REPLACE FUNCTION substract_datavalue_between_non_repeatable_stages(
    ...
    _de_src2   VARCHAR(50),
    _ps_src2   VARCHAR(11),
    _de_target VARCHAR(50),
    _ps_target VARCHAR(11))
...
DECLARE substract value_with_date;
		...
		select psi.* into event_src2 from programstageinstance psi 			
			...
			IF  event_src1.programstageinstanceid is not null and event_src2.programstageinstanceid is not null 
				...
				aux_datavalue_src1 = getDataValueInStageByProgramInstance(
				_pi_id,
				_ps_src1,
				_de_src1
				);		
				
				aux_datavalue_src2= getDataValueInStageByProgramInstance(
				_pi_id,
				_ps_src2,
				_de_src2
				);		
